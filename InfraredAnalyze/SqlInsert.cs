            ...
            while (true)
            {
                if (TemperDataLength > 0)
                {
                    ...
                    for (int i=0;i<tempMessage.len;i++)
                    {
                        ...
                        {
                            if (StaticClass.intPtrs_Status[CameraId - 1] == 1)//测温告警 1
                            {
                            ...
                            else if (StaticClass.intPtrs_Status[CameraId - 1] == 3)//正常 
                            {
                                sqlCreate.Insert_TemperData(CameraId, tempMessage.dvrIP, dateTime, tagTemperature.type, tagTemperature.number, tagTemperature.temper, "正常");
                            }
                        }
                    }
